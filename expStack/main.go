...
func main() {
	...
	for {
		...
		if IsOper(temp) {          //说明是符号
			...
			if operStack.Top == -1 { //空栈
				...
				//就从符号栈pop出，并从数栈也pop两个数，进行运算，运算后的结果在重新入栈到数栈，当前准备入栈的符号再入到符号栈
				if operStack.Priority(operStack.arr[operStack.Top]) >= operStack.Priority(temp) {
					num1, _ = numStack.Pop()
					num2, _ = numStack.Pop()
					oper, _ = operStack.Pop()
					result = operStack.Cal(num1, num2, oper)
					//将计算的结果重新入数栈，当前准备入栈的符号再入到符号栈
			...
			keepNum += ch
			//如果已经到表达式最后，直接将keepNum转成int64压入数栈
			if index == len(exp)-1 {
	...
	res, _ := numStack.Pop()
	fmt.Printf("%v最后的运算结果为：%v", exp, res)
